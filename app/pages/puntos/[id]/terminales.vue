...
const { data, pending, error } = await useAsyncGql({
    operation: 'GetTerminalesByPunto',
    variables: { puntoId }
});
...
const filteredTerminales = computed(() => {
    if (!data.value?.terminalesByPunto) return [];

    return data.value.terminalesByPunto.filter((terminal) => {
        const matchesActivo = filterActivo.value === '' ||
            (filterActivo.value === 'true' ? terminal.activo : !terminal.activo);
        return matchesActivo;
    });
});

const totalPages = computed(() => Math.ceil(filteredTerminales.value.length / pageSize));

const paginatedTerminales = computed(() => {
    const start = (currentPage.value - 1) * pageSize;
    return filteredTerminales.value.slice(start, start + pageSize);
});
...
<template>
    <div class="flex flex-col gap-6">
        <div class="flex flex-col items-center gap-2 px-4">
            <h2 class="text-3xl font-bold text-center">Terminales</h2>
            <p class="text-gray-600 text-center">
                Terminales de votación asignadas al punto.
            </p>
        </div>
        ...
        <div class="max-w-6xl w-full mx-auto">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div v-if="pending" class="text-center py-8">
                    <p class="text-gray-500">Cargando terminales...</p>
                </div>

                <!-- Error State -->
                <div v-else-if="error" class="text-center py-8">
                    <p class="text-red-500">Error al cargar las terminales</p>
                </div>

                <!-- Empty State -->
                <div v-else-if="filteredTerminales.length === 0" class="text-center py-8">
                    <p class="text-gray-500">No se encontraron terminales</p>
                </div>

                <!-- Terminales List -->
                <div v-else>
                    <div class="grid gap-4">
                        <div v-for="terminal in paginatedTerminales" :key="terminal.id"
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 ml-5 text-sm text-gray-600 space-y-1">
                                        <p>
                                            ...
                                            <span :class="terminal.activo ? 'text-green-600' : 'text-gray-400'">
                                                {{ terminal.activo ? 'Activa' : 'Inactiva' }}
                                            </span>
                                        </p>
                                        <p>
                                            <span class="font-medium">Clave pública:</span>
                                            <span class="font-mono text-xs">
                                                {{ terminal.clavePublica.substring(0, 20) }}…
                                            </span>
                                        </p>
                                    </div>
                                </div>
                                <NuxtLink :to="`/terminales/${terminal.id}`"
